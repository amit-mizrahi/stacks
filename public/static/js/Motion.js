...
var Motion = {
  xFlight: function(game, stacks, source, target) {
    var s = source.size();
    var r = target.size();
  ...
  yFlight: function(game, stacks, source, target) {
    var s = source.size();
    var r = target.size();
    var g = Physics.GRAVITY;
    var h = Geometry.ELEMENT_HEIGHT;
    ...
    return Math.pow(-0.5*g*(timeElapsed), 2) + (0.5*g*T - (Geometry.GROUND_HEIGHT + Geometry.MARKER_HEIGHT + (h*s))/T +
      (Geometry.GROUND_HEIGHT + Geometry.MARKER_HEIGHT + (h*r))/T)*timeElapsed + (Geometry.MARKER_HEIGHT + Geometry.GROUND_HEIGHT + h*s);
  },

  isColliding: function(flyingElement, stackTopX, stackTopY, dy) {
